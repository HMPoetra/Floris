...
import { getWhatsAppLink, getPngBlobFromUrl, copyBlobToClipboard, uploadImageToLink } from "../utils/whatsapp";
...
export default function HighlightProduct() {
  ...
  useEffect(() => {
    ...
    const fetchBlob = async () => {
      const fullUrl = window.location.origin + selectedProduct.images[0];
      const blob = await getPngBlobFromUrl(fullUrl);
      if (isMounted) {
        setImageBlob(blob);
      }
    };
  ...
  const handleOrderClick = async () => {
    if (isOrdering || !selectedProduct) return;
    setIsOrdering(true);
    ...
    if (newTab) {
      newTab.document.write(`
        <html>
          <head>
            <title>Menyiapkan Pesanan...</title>
            <style>
              body {
                display: flex;
              ...
              .spinner {
                border: 4px solid rgba(217, 162, 153, 0.1);
                width: 36px;
                height: 36px;
                border-radius: 50%;
    ...
    try {
      const fullUrl = window.location.origin + selectedProduct.images[modalImageIndex];
      
      // 1. Upload highlight product image to temporary public hosting for link preview
      const publicImageUrl = await uploadImageToLink(fullUrl);
      
      // 2. Clipboard copy as fallback/backup
      let blobToCopy = imageBlob;
      if (!blobToCopy) {
        blobToCopy = await getPngBlobFromUrl(fullUrl);
      }
      if (blobToCopy) {
        await copyBlobToClipboard(blobToCopy);
        setShowToast(true);
        setTimeout(() => setShowToast(false), 4000);
      }
      
      // 3. Open WhatsApp with order details and public image link
      const finalImageUrl = publicImageUrl ? encodeURI(publicImageUrl) : encodeURI(fullUrl);
      const waText = `Halo Jalé Florist, saya tertarik dengan produk ${selectedProduct.name} (${selectedProduct.price}).\n\n*Gambar Produk:* ${finalImageUrl}\n\nBoleh minta info lebih lanjut?`;
      const waUrl = `${getWhatsAppLink()}?text=${encodeURIComponent(waText)}`;
      if (newTab) {
      ...
      console.error('Error handling highlight order:', error);
      // Fallback in case of upload failure
      const fallbackUrl = encodeURI(window.location.origin + selectedProduct.images[modalImageIndex]);
      const waText = `Halo Jalé Florist, saya tertarik dengan produk ${selectedProduct.name} (${selectedProduct.price}).\n\n*Gambar Produk:* ${fallbackUrl}\n\nBoleh minta info lebih lanjut?`;
      const waUrl = `${getWhatsAppLink()}?text=${encodeURIComponent(waText)}`;
      if (newTab) {
        newTab.location.href = waUrl;
      } else {
  ...
  return (
    ...
    >
            ...
            <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col bg-white overflow-y-auto">
              ...
              <div className="mt-auto pt-6 border-t border-sand/40">
                <button
                  ...
                  disabled={isOrdering}
                  className="w-full inline-flex justify-center items-center gap-2 bg-[#25D366] hover:bg-[#1DA851] text-white font-bold px-8 py-4 rounded-xl transition-all shadow-md hover:shadow-lg hover:-translate-y-1 cursor-pointer disabled:opacity-75 disabled:cursor-wait"
                >
                  {isOrdering ? (
                    <>
      ...
      {showToast && (
        <div
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[1000] bg-charcoal/95 text-cream px-6 py-3 rounded-full shadow-2xl flex items-center gap-2.5 backdrop-blur-md transition-all duration-300 border border-sand/20"
          style={{ animation: 'slideUpFadeIn 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards' }}
        >
          <svg className="w-5 h-5 text-green-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
          </svg>
          <span className="text-xs md:text-sm font-medium tracking-wide">
            Gambar disalin! Silakan tempel (Ctrl+V) di WhatsApp 📋
          </span>
        </div>
      )}
